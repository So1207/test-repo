...
chat_id1 = 990167577 # bot.getUpdates()[-1].message.chat.id
...
while True:
	...
	now = datetime.now()
	url='http://www.cgv.co.kr/common/showtimes/iframeTheater.aspx?areacode=05,207&theatercode=0128&date='+ str(date) +'&screencodes=&screenratingcode=02&regioncode=cf'
	...
	# 영화제목이 크롤링 데이터 내에 있는지 확인후 메세지 전송
		if str('IMAX') in html:
			bot.sendMessage(chat_id = chat_id1, text = title + "%d IMAX 예매오픈!" % date)
			confirm_date = date
		print(now)
		print('date =', date)
		print('정상작동중')
	...
	# 매일 1회 정상작동 메세지 출력, 이후 1분간 휴식
		...
		if(now.hour == 12 and now.minute == 40):
			bot.sendMessage(chat_id = chat_id1, text = "%d:%d 아맥알리미 정상작동중!" % (now.hour, now.minute))
			time.sleep(60)
		print(now)
		print('date =', date)
		print('정상작동중')
